...
fn draw_latency(f: &mut Frame, app: &App, area: Rect) {
    ...
    let warn_pts: Vec<(f64, f64)> = if show_warn {
        (0..ring_len.max(1)).map(|i| (i as f64, lwarn)).collect()
    } else {
        Vec::new()
    };
...
fn draw_jitter(f: &mut Frame, app: &App, area: Rect) {
    ...
    let show_warn = jwarn <= y_max;
    let warn_pts: Vec<(f64, f64)> = if show_warn {
        (0..ring_len.max(1)).map(|i| (i as f64, jwarn)).collect()
    } else {
        Vec::new()
    };
    let mut datasets: Vec<Dataset> = Vec::new();
...
fn rssi_verdict(rssi: i16) -> &'static str {
    if rssi >= -55 {
        "excellent"
    } else if rssi >= -67 {
        "good"
...
fn draw_stats(f: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(rssi) = app.wifi_rssi {
        let bars = rssi_to_bars(rssi);
        let color = rssi_color(rssi);
        let mut bar_str = String::new();
        for i in 0..5u8 {
            bar_str.push(if i < bars { '▮' } else { '▯' });
        }
        lines.push(Line::from(vec![
            Span::styled(" wifi ", Style::default().fg(MUTED)),
            ...
            Span::raw("   "),
            Span::styled(rssi_verdict(rssi), Style::default().fg(color)),
        ]));
    }
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:.0} ms", peak),
            Style::default().fg(if peak > app.lat_bad_ms() { BAD } else { OK }),
        ),
        Span::raw("   "),
        Span::styled(" peak jit ", Style::default().fg(MUTED)),
        Span::styled(
            format!("{:.0} ms", peak_jit),
            Style::default().fg(if peak_jit > app.jit_warn_ms() {
                WARN
            } else {
                OK
            }),
