...
impl PrimaryProbe {
    ...
    fn adaptive_thresholds(&self, cfg: &Config) -> (f64, f64, f64, f64) {
        ...
        let lat_thresh = (lat_p90 * 1.5).max(cfg.latency_warn_ms);
        let jit_thresh = (jit_p90 * 2.0).max(cfg.jitter_warn_ms);
        (
            lat_thresh,
            jit_thresh,
            cfg.degraded_loss_pct,
...
impl App {
    ...
    pub fn score(&self) -> f32 {
        ...
        if lat > self.cfg.latency_warn_ms as f32 {
            s -= ((lat - self.cfg.latency_warn_ms as f32) / 10.0).min(30.0);
        }
        if jit > self.cfg.jitter_warn_ms as f32 {
            s -= ((jit - self.cfg.jitter_warn_ms as f32) / 5.0).min(20.0);
        }
        s -= loss.min(40.0);
        if dns > self.cfg.dns_warn_ms as f32 {
            s -= ((dns - self.cfg.dns_warn_ms as f32) / 10.0).min(10.0);
        }
